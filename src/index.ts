...
class PolymarketCopyBot {
  ...
  async initialize(): Promise<void> {
    ...
    console.log(`   Alchemy WS: ${config.alchemy.enabled ? 'Enabled' : 'Disabled'}`);
  ...
  async start(): Promise<void> {
    ...
    if (this.wsMonitor) monitoringMethods.push('Polymarket WS');
    if (this.alchemyMonitor) monitoringMethods.push('Alchemy WS');
  ...
  stop(): void {
    ...
    if (this.alchemyMonitor) {
      this.alchemyMonitor.close();
    }
